...
class ServiceCategory_Controller extends Page_Controller {
	...
	public function getService(){
		...
		//Debug::show($Category);
		$ServicesList = ServicePage::get()->filter('ParentID', $this->ID)->sort('"Sort" ASC, "ID" ASC')->limit($this->page_length * $this->page_num) ;
		
		$row_count = ServicePage::get()->filter('ParentID', $this->ID)->count();
	...
	public  function AjaxGetMore(SS_HTTPRequest $request){
		...
		$ItemDLP= ServicePage::get()->filter('ParentID', $this->ID)->sort('"Sort" ASC, "ID" DESC')->limit($page_length, $start_num);

...
class ProductListPage_Controller extends Page_Controller {
	...
	public function getProduct(){
		...
		//Debug::show($Category);
		$ProductsList = ProductPage::get()->filter('ParentID', $Category->ID)->sort('"Sort" ASC, "ID" ASC')->limit($this->page_length * $this->page_num) ;
		
		$row_count = ProductPage::get()->filter('ParentID', $Category->ID)->count();
	...
	public  function AjaxGetMore(SS_HTTPRequest $request){
		...
		$ItemDLP= ProductPage::get()->filter('ParentID', $Category->ID)->sort('"Sort" ASC, "ID" DESC')->limit($page_length, $start_num);

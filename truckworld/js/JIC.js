...
var jic = {
        ...
        compress: function(source_img_obj, quality,resizeWidth){
             var cvs = document.createElement('canvas');
             cvs.width = resizeWidth;
             cvs.height = source_img_obj.naturalHeight * (resizeWidth/source_img_obj.naturalWidth);
             var ctx = cvs.getContext("2d").drawImage(source_img_obj, 0, 0, resizeWidth, source_img_obj.naturalHeight * (resizeWidth/source_img_obj.naturalWidth));
        ...
        rotate: function(source_img_obj, degrees){
             var cvs = document.createElement('canvas');

 			 if(imgMyEven){//even times, width and height are the same.
	 			 cvs.width = source_img_obj.naturalWidth;	
	 			 cvs.height = source_img_obj.naturalHeight;
	 			 
	 			 imgMyEven = false;
 			 }else{//odd times, swap the width and height.
	 			 cvs.width = source_img_obj.naturalHeight;	
	 			 cvs.height = source_img_obj.naturalWidth;

	 			 imgMyEven = true;
 			 }
 			 
             var imgOw = source_img_obj.naturalWidth;
             var imgOh = source_img_obj.naturalHeight;
             
             var ctx=cvs.getContext("2d");
             ctx.clearRect(0,0,imgOw,imgOh);

             ctx.save();
             
             ctx.translate(cvs.width/2,cvs.height/2);
             ctx.rotate(degrees*Math.PI/180);
             ctx.drawImage(source_img_obj,-source_img_obj.naturalWidth/2,-source_img_obj.naturalHeight/2);
             ctx.restore();

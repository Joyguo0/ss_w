...
//auto read database username, password, database name from .my.cnf if file exist.
$handle = @fopen("../../.my.cnf", "r");
if ($handle) {
	global $databaseConfig;
    ...
    while (($buffer = fgets($handle, 4096)) !== false) {
        if(strpos($buffer, "ser=") > 0 ){
        	$term = explode("user=", $buffer);
        	$username = $term[1];
        	$username = trim($username);
        }
       	
        if(strpos($buffer, "assword=") > 0 ){
        	$term = explode("password=", $buffer);
        	$password = $term[1];
        	$password = trim($password);
        }
        	
        if(strpos($buffer, "atabase=") > 0 ){
        	$term = explode("database=", $buffer);
        	$database = $term[1];

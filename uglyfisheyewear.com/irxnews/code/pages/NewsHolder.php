...
// 		$this->extracrumbs[] = $page;
	...
	public function CheckActiveCategoryID(){
	...
	public function NewsCategorys(){
		$ItemDLP = NewsCategory::get();
		$NewCategoryList = new ArrayList();
		if($ItemDLP && $ItemDLP->Count()){
			foreach ($ItemDLP as $itemDO){
		
				$itemDO->paramID = $this->owner->request->param('ID');
				$NewCategoryList->push($itemDO);
			}
		}
		return $NewCategoryList;
	}
	
	public function CategorysSeach(){
		$categoryID = $this->owner->request->param('ID');
		return $this->owner->customise(array(
			'News' => News::get()->filter(array('NewsCategoryID' => $categoryID)),
		));
	}
